  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('LspAttach', {
      ...
      callback = function(event)
        ...
        -- Don't attach to decompiled files to avoid lsp exceptions.
        local attachingTo = vim.api.nvim_buf_get_name(event.buf)
        if Starts(attachingTo, '/$metadata$/') and vim.bo[event.buf].filetype == 'cs' then
          fidget.notify 'Omnisharp not attaching to decompiled file.'
          return
        end
        ...
        local function safeHighlight()
          if not pcall(vim.lsp.buf.document_highlight) then
            fidget.notify('highlight failed', vim.log.levels.INFO)
          end
        end

        local function safeClearReferences()
          if not pcall(vim.lsp.buf.clear_references) then
        ...
        if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight) then
          ...
          vim.api.nvim_create_autocmd({ 'CursorHold', 'CursorHoldI' }, {
            ...
            callback = safeHighlight,
          })

          vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI' }, {
            buffer = event.buf,
            group = highlight_augroup,
            callback = safeClearReferences,
          })
    ...
    -- Add formatters and other tools here,
    ensure_installed =
      {
        'bash',
        'c',
